...
void Constants::SetConstants(void)
{
	...
	Constants::lsun_cgs= (3.839e26)*GSL_CONST_CGSM_JOULE;//solar luminosity in erg/s
...
/*! \fn alpha_B_HII(double T)
    \brief case B HII recombination coefficient
*/
double Constants::alpha_B_HII(double T)
{
	//case B HII recombination coefficient
	//T is in Kelvin
	//appendix A of Hui & Gnedin 1997a
	double T_TR = 157807.0; //K
	double lambda_HI = 2.0*T_TR/T;

	//cm^3 s^-1	
	return 2.753e-14 * pow(lambda_HI,1.5)/pow( 1.0 + pow(lambda_HI/2.740, 0.407), 2.242);
}
